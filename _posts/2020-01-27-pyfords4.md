...
이번 장에서는 `pandas`,`matplotlib`, `seaborn` 패키지를 활용하여 데이터를 시각화하는 기본적인 방법들을 다룹니다. 먼저 씨본의 다양한 플롯과 차트들을 다룬 후, 맷플롯의 기능들을 공부하도록 하겠습니다. 씨본은 맷플롯을 기반으로 만들어진 패키지이만 씨본이 더 쉽고 직관적인 인터페이스를 제공하기 때문에 씨본을 먼저 다루도록 하겠습니다.
